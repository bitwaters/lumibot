...
## Purpose
聊天消息内嵌合约地址时自动识别并返回 token 查询卡：链自动判定、数据复用信号管道、查询节流防滥用。
