...
signal 与 trending 候选 MUST 均通过 token info 补查 `visiting_count`（优先缓存，300s TTL），并 MUST 达到该链 visiting 阈值后才可告警；载荷中的 visiting 值 MUST NOT 作为门禁依据。signal 与 trending 候选统一按该链 `filters.visiting_min`（一套阈值，不区分源）。visiting 补查失败或无法解析时 MUST 拒绝（fail-closed）。
